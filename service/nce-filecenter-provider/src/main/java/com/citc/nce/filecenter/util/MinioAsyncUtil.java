...
import com.citc.nce.filecenter.util.model.FileUploadInfo;
import com.citc.nce.filecenter.util.model.UploadUrlsVO;
...
public class MinioAsyncUtil {
    ...
    private final String bucket = "robot";
    ...
    public UploadUrlsVO getUploadObjectUrl(String contentType, String object) {
        try {
            ...
            throw new BizException("单文件上传失败");
        }
    }
    ...
    public UploadUrlsVO initMultiPartUpload(FileUploadInfo fileUploadInfo, String object) {
        ...
        try {
            ...
            // 返回 文件上传失败
            throw new BizException("文件初始化分片上传失败");
        }
    }
    ...
    public boolean mergeMultipartUpload(String object, String uploadId) {
        log.info("通过 <{}-{}-{}> 合并<分片上传>数据", object, uploadId, bucket);
        // 获取所有分片
