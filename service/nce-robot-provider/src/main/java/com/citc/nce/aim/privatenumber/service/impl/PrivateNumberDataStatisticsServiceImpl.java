...
@Service
public class PrivateNumberDataStatisticsServiceImpl implements PrivateNumberDataStatisticsService {
    ...
    public PrivateNumberDataStatisticsResp dataStatistics(PrivateNumberDataStatisticsReq req) {
        PrivateNumberDataStatisticsResp resp = new PrivateNumberDataStatisticsResp();
...
//        List<String> sendTimeList = aimSentDataDao.findSendTimeListByProjectIdAndTime(req.getProjectId(),DateUtils.obtainDateStr(startDate),DateUtils.obtainDateStr(endDate));
         ...
         * 2023-10-19
         */
        LambdaQueryWrapperX<PrivateNumberSentDataDo> queryWrapperX = new LambdaQueryWrapperX<>();
        queryWrapperX.ge(PrivateNumberSentDataDo::getCreateTime, startDate).le(PrivateNumberSentDataDo::getCreateTime, endDate).eq(PrivateNumberSentDataDo::getProjectId, req.getProjectId());
        List<PrivateNumberSentDataDo> dataDos = privateNumberSentDataDao.selectList(queryWrapperX);
        if (CollectionUtils.isEmpty(dataDos)) {
            return resp;
        }
        List<String> sendTimeList = dataDos.stream().map(data -> DateUtils.obtainDateStr(data.getCreateTime())).collect(Collectors.toList());
        //2、根据时间类型分类
        String[] timeArr;
        String key;
